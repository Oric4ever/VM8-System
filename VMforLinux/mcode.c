...
// new opcodes
    ...
    case 27: typecheck(WFETCH, pop(), pop()); break; // TYPCHK
    case 28: op2 = pop(); op = pop(); push(bios(op,op2)); break; // SYS
    case 29: iop2 = ipop(); iop = ipop(); push(iop>=0 ? iop%iop2 : (iop % iop2 + iop2) % iop2); break; // Modulus
...
bool mcode_interp(void)
{
  ...
  if (TRACE_ALL) {
    ...
    fprintf(tracelog," [FE0A]=%04x",*WMEM(0xFE0A));
...
//    else
  ...
  case 0xb9: itmp=ipop(); iop=ipop(); push(iop>=0 ? iop/itmp : iop/itmp + (iop%itmp==0)); break;
